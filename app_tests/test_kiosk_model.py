...
class KioskModelTestCase(unittest.TestCase):
    ...
    def testUpdatingCachedEntityFromCache(self):
        """Get an entity from cache, change it and put it back. Compare with db entity."""
        a = memcache.get('A', namespace="kiosks")
        a.name = "Boo"
        a.put()
        b = db.GqlQuery("SELECT * FROM Kiosk WHERE dieid = :1", 'A').get()
        self.assertEqual(a.name, b.name)
        self.assertEqual(a.is_saved(), True)